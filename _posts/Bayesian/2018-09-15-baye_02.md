...
- y는 bernoulli 특성에 따라 아래와 같은 binomial로 정의할 수 있다. (likelihood 관점)
...
- $\theta$가 $[0,1]$의 임의의 값(uniform = $\frac{1}{b-a}$)을 가진다고 가정해보자. (prior 관점)
	- prior를 uniform분포(beta(1,1))로 가정할 경우 posterior가 beta distribution를 따른다는 것을 미리 알 수 있다는 이점이 있다.
