...
```A fixed-window	neural Language Model```은 고정된 **small window size**와 학습되는 $\mathbf{W}_h$ **weight matrix가 공유되지 않는다는 점**에서 한 대안으로 RNN을 제시하였다.
...
- 각 $t$-step에 대한 loss는 아래와 그림 같이 표현된다.
	- 모델은 다음의 단어의 확률이 최대화되는 negative log-liklihood를 최소화하는 것이 목적이다.
	- 그리고, 우리의 목적함수는 softmax를 0~1의 값을 가지기 때문에, 일반적으로 ```target distribution```($y_j$)을 ```bernoulli distribution```을 가정하게 된다.
	- 이렇게 ```bernoulli distribution```을 가정하게 되면, negative log-liklihood는 아래와 같은 cross-entropy의 형태로 사용될 수 있다. (반대로, ```gaussian distribution```을 가정할 경우, mean square error의 형태가 된다.)

$$
\begin{align}
\hat{y}_{j} &= \dfrac{e^{f_j}}{\sum_{i} e^{f_i}}, \quad y_j \sim \text{Bernoulli}(\hat{y}_{j})\\
P(y_j=1) &= \hat{y}_{j}^{y_j} (1-\hat{y}_{j})^{1-y_j} \quad \text{for } y_j\in\{0,1\}\\
L(p) &= -\log\prod^{|V|}_{j=1} P(y_j=1) \\
L(p) &= -\log\prod^{|V|}_{j=1} \hat{y}_{j}^{y_j} (1-\hat{y}_{j})^{1-y_j} \\
\end{align}
$$
