...
- 그리고 ```window_size```가 3, ```embedded_dim```이 4라고 가정하면 아래와 같은 그림과 notation으로 표현할 수 있다. 
    - $diag(\mathbf{m}_{:,m}) \in \mathbb{R}^{4 \times 4}$ 이라고 하면은 $\mathbf{M} \in \mathbb{R}^{3 \times 4\times 4} $ 
...
<p align="center"><img width="700" height="auto" src="https://imgur.com/MD7zDdd.png"></p>
